...
def do_train(
    ...
    for iteration, batch in enumerate(data_loader, start_iter):
        ...
        if cfg.MODEL.DDTNET_ON:
            ...
            pt_map = [pt_map.to(device) for pt_map in batch[4]]
            center = [center.to(device) for center in batch[5]]
            bkg = [bkg.to(device) for bkg in batch[6]]
            obj = [obj.to(device) for obj in batch[7]]

            loss_dict = model(images, targets, gt, center,pt_map,bkg,obj)
        ...
        con_loss = loss_dict.get('loss_con')
        cam_loss = loss_dict.get('loss_cam')
        ...
        if iteration >= 1000:
            losses = a * dec_loss + b * seg_loss + con_loss    + cam_loss 
        else:
            losses = a * dec_loss + b * seg_loss + con_loss*0  + cam_loss * 0 
        ...
        dec_losses_reduced = loss_dict_reduced.get('loss_cls') + loss_dict_reduced.get('loss_reg') + loss_dict_reduced.get('loss_centerness')
        seg_losses_reduced = loss_dict_reduced.get('loss_mask')
        con_losses_reduced = loss_dict_reduced.get('loss_con')
        cam_losses_reduced = loss_dict_reduced.get('loss_cam')

        losses_reduced = a * dec_losses_reduced + b * seg_losses_reduced + con_losses_reduced + cam_losses_reduced
        meters.update(loss=losses_reduced, **loss_dict_reduced)
        ...
        if args.eval_step > 0 and iteration % args.eval_step == 0 and not iteration == max_iter:
            ...
            if get_rank() == 0 and summary_writer:
                ...
                if f1 > max_AP :
                    checkpointer.save("F1model_{:05d}_{:04f}".format(iteration,f1), **arguments)
                    max_AP = f1

                dice = eval_result['metrics1'].get('dice')
                if dice > max_dice :
                    checkpointer.save("Dicemodel_{:05d}_{:04f}".format(iteration,dice), **arguments)
                    max_dice = dice
                ...
                if cfg.MODEL.DDTNET_ON:
                    ...
                    if f1 > 0.2:
                        aji = eval_result['metrics1'].get('aji')
                        if aji > max_aji :
                            checkpointer.save("AjiModel_{:05d}_{:04f}".format(iteration,aji), **arguments)
                            max_aji = aji
