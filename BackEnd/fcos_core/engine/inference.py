...
def compute_on_dataset(cfg, model, data_loader, device, timer=None):
    ...
    for iter, batch in enumerate(tqdm(data_loader)):
        images, targets, masks, image_id, pt_map, centers,bkg,obj = batch
        
        targets = [target.to(device) for target in targets]
        mask = [mask.to(device) for mask in masks]
        center = [center.to(device) for center in centers]
        pt_map = [i.to(device) for i in pt_map]
        bkg = [i.to(device) for i in bkg]
        obj = [i.to(device) for i in obj]
        ...
        with torch.no_grad():
            ...
            if cfg.MODEL.DDTNET_ON :
                output, mask, losses = model(images.to(device), targets, mask, center, pt_map, bkg, obj)
                vallosses = sum(loss for loss in losses.values())
            else:
                output, losses = model(images.to(device), targets)
                vallosses = sum(loss for loss in losses.values())
            if timer:
                torch.cuda.synchronize()
                timer.toc()
            bbox = [o.to(cpu_device) for o in output]
            preds = [o.to(cpu_device) for o in mask[0]]
        
        # results_dict.update(
        #     {img_id: result for img_id, result in zip(image_id, bbox)}
        # )
        results_dict.update(
            {img_id: [result,pred] for img_id, result, pred in zip(image_id, bbox, preds)}
        )
