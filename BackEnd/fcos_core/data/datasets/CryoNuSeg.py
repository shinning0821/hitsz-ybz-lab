...
class CryoNuSegDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, data_name, split, use_difficult=False,transforms=None):
        ...
        self.ids = CryoNuSegDataset._read_image_ids(image_sets_file)
        self.id_to_img_map = {k: v for k, v in enumerate(self.ids)}
        cls = CryoNuSegDataset.CLASSES
    ...
    def map_class_id_to_class_name(self, class_id):
        return CryoNuSegDataset.CLASSES[class_id]


class CryoNuSegTestDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, data_name, split, use_difficult=False,transforms=None):
        ...
        self.transforms = transforms
        image_sets_file = os.path.join(self.data_dir, "%s.txt" % (self.split))
        self.ids = CryoNuSegTestDataset._read_image_ids(image_sets_file)

        self.id_to_img_map = {k: v for k, v in enumerate(self.ids)}

        cls = CryoNuSegTestDataset.CLASSES
        self.class_to_ind = dict(zip(cls, range(len(cls))))
        self.coco = COCO(os.path.join(self.data_dir, 'annotation.json'))
    ...
    def map_class_id_to_class_name(self, class_id):
        return CryoNuSegTestDataset.CLASSES[class_id]
