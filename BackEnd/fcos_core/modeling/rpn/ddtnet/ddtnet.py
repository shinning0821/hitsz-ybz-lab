...
class MultiHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        bbox_tower_list = []
        cls_tower_list = []
        for _ in range(num_bbox_towers):
            bbox_tower = []
            cls_tower = []
            for i in range(cfg.MODEL.DDTNet.NUM_CONVS):
                bbox_tower.append(
            ...
            cls_tower.append(nn.GroupNorm(32, in_channels))
            cls_tower.append(nn.ReLU())
            bbox_tower_list.append(nn.Sequential(*bbox_tower))
            cls_tower_list.append(nn.Sequential(*cls_tower))
        
        self.bbox_towers = nn.ModuleList(bbox_tower_list)
        self.cls_towers = nn.ModuleList(cls_tower_list)
        ...
        self.bbox_preds = nn.ModuleList(bbox_pred_list)
        self.cls_logits = nn.ModuleList(cls_logits_list)
        ...
        # initialization
        for modules in [self.mask_tower, self.centerness, self.mask, self.maskchannel,
                        self.cls_towers, self.bbox_towers,self.bbox_preds,
                        self.cls_logits]:
            for l in modules.modules():
                if isinstance(l, nn.Conv2d):
                    torch.nn.init.normal_(l.weight, std=0.01)
    ...
    def forward(self, x):
        ...
        for l, feature in enumerate(x):
            ...
            if l == 2 or l == 3 or l == 4 or l == 5 or l == 1:  # 大哥你不同层级共用一个检测头的啊。。。
                cls_tower = self.cls_towers[l-1](feature)
                box_tower = self.bbox_towers[l-1](feature)

                logits.append(self.cls_logits[l-1](cls_tower))
                if self.centerness_on_reg:
                    centerness.append(self.centerness(box_tower))
                else:
                    centerness.append(self.centerness(cls_tower))
                ...
                bbox_pred = self.scales[l-1](self.bbox_preds[l-1](box_tower))
                if self.norm_reg_targets:
                    bbox_pred = F.relu(bbox_pred)
                    if self.training:
                        bbox_reg.append(bbox_pred)
...
class DDTNETModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None, gt=None, center=None):
        """
        Arguments:
        ...
        if self.training:
            ...
            return self._forward_test(
                locations, box_cls, box_regression,
                centerness, images.image_sizes, targets, mask, gt, center
            )

    def _forward_train(self, locations, box_cls, box_regression, centerness, targets, mask, gt, center):
        loss_box_cls, loss_box_reg, loss_centerness, loss_mask = self.loss_evaluator(
            locations, box_cls, box_regression, centerness, targets, mask, gt, center
        )
        losses = {
    ...
    def _forward_test(self, locations, box_cls, box_regression, centerness, image_sizes, targets, mask, gt, center):
