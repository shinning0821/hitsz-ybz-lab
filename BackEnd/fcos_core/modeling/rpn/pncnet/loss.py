...
class DDTNETLossComputation(object):
    ...
    def compute_targets_for_locations(self, locations, targets, object_sizes_of_interest):
        ...
        for im_i in range(len(targets)):
            ...
            if(bboxes.shape[0] == 0):
                bboxes = torch.tensor([0, 0, 2, 2]).unsqueeze(0).to(torch.float32).cuda()
                labels_per_im = torch.zeros(1).cuda()
                area = torch.tensor([4]).cuda()
            ...
            if(locations_to_gt_area.shape[1]==0):
                # print("warning : there is an empty boxlist")
                locations_to_min_area = torch.zeros(locations_to_gt_area.shape[0]).to(torch.int64).cuda()
                locations_to_gt_inds = torch.ones(locations_to_gt_area.shape[0]).to(torch.int64).cuda()
                reg_targets_per_im = torch.tensor([0, 0, 2, 2]).repeat(locations_to_gt_area.shape[0], 1).to(torch.float32).cuda()
                labels_per_im = torch.zeros(locations_to_gt_area.shape[0]).cuda()
    ...
    def dice_loss(self, mask, gt):
        smooth = 1e-5
        mask = mask.view(-1)
        gt = gt.view(-1)
        intersection = (mask * gt).sum()
        union = mask.sum() + gt.sum()
        dice = (2. * intersection + smooth) / (union + smooth)
        loss = 1 - dice
        return loss
    
    def weak_bce_loss(self, pred, pt_map, bkg):
        f_loss = F.cross_entropy(pred, pt_map, 
                        ignore_index=0)
        b_loss = F.cross_entropy(pred, 1-bkg, 
                            ignore_index=1) 
        # print(pred.shape, pt_map.shape, bkg.shape)
        return f_loss + b_loss
    
    def obj_loss(self, mask, pt_map, obj):
        prob = torch.softmax(mask, dim=1)
        eps = 1e-6
        b_loss = torch.mean(-(1-obj)*torch.log(eps+prob[:, 0,:,:]))

        f_loss = 0
        for cls in torch.arange(1, 2):
            f_loss += -obj*(pt_map == cls)*torch.log(eps+torch.sum(prob[:, 1:], dim=1))
        f_loss = torch.mean(f_loss) 
        
        return b_loss 

    def mask_loss(self, mask, gt, pt_map=None, bkg=None, obj=None):
        ...
        GT = torch.stack(gt).squeeze(1).long()
  
        full_loss = F.cross_entropy(mask160, GT) 

        pt_map = torch.stack(pt_map).squeeze(1).long()
        bkg = torch.stack(bkg).squeeze(1).long()
        obj = torch.stack(obj).squeeze(1)
        obj /= 255

        # 简单的根据这两种像素的粗略标签进行预测
        ce_loss = self.weak_bce_loss(mask160,pt_map,bkg)

        # 再计算obj loss
        obj_loss = self.obj_loss(mask160,pt_map,obj)
        
        weak_loss = ce_loss 

        return weak_loss

    def __call__(self, locations, box_cls, box_regression, centerness, targets, mask, gt=None,center=None,pt_map = None,bkg=None,obj=None):
        """
        Arguments:
...
def make_pncnet_loss_evaluator(cfg):
